layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url: web.rootPath() + 'custLinkman/list',
        cellMinWidth: 95,
        page: true,
    ...
    table.on('toolbar(List-toolbar)', function (obj) {
        if (obj.event == 'add') {
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: web.rootPath() + 'custLinkman/add.html'
            });
        }
        ;
    });
        ...
        switch (obj.event) {
            case 'update':
                layer.open({
                    ...
                    fixed: false,
                    maxmin: true,
                    content: web.rootPath() + "custLinkman/" + data.id + ".html?_"
                });
                break;
            case 'delete':
                var index = layer.confirm('确定要删除?', {
                    ...
                    $.ajax({
                        url: web.rootPath() + "custLinkman/delete/" + data.id,
                        type: "delete",
